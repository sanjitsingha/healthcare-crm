      ...
      {open && (
        <>
          ...
          <div ref={menuRef} className="fixed z-50 overflow-y-auto rounded-lg border border-(--color-border) py-1"
            ...
            {options.length === 0 && <p className="px-3 py-2 text-[12px]" style={{ color: 'var(--color-text-muted)' }}>No options</p>}
            {options.map(o => {
...
export default function FollowupTable({
  ...
  // Excel-like cell: every cell shows a right + bottom gridline.
  const cell = (children, extra = '') => (
    <div className={`${cellBase} ${extra}`} style={{ borderRight: `1px solid ${GRID}`, borderBottom: `1px solid ${GRID}` }}>
      {children}
    </div>
  )
    ...
    <div className="overflow-x-auto" style={{ borderTop: `1px solid ${GRID}` }}>
      <div style={{ minWidth: '1030px' }}>
        ...
        <div className="grid" style={{ gridTemplateColumns: FU_COLS, background: 'var(--color-surface-2)' }}>
          {head.map(h => (
            <div key={h} className="px-2.5 py-2.5 text-[11px] font-800 uppercase tracking-wider"
              style={{ color: 'var(--color-text-secondary)', borderRight: `1px solid ${GRID}`, borderBottom: `2px solid ${GRID}` }}>{h}</div>
          ))}
        </div>
        {/* Data rows */}
        {sorted.map(f => (
          <div key={f.id} className="grid items-stretch" style={{ gridTemplateColumns: FU_COLS }}>
            {cell(<ChipCell value={f.type} options={types} styleFor={(v) => typeStyle[v]} onChange={(v) => onField(f.id, { type: v })} />)}
            {cell(<TextCell value={toLocalInput(f.scheduled_at)} type="datetime-local" onCommit={(v) => onField(f.id, { scheduled_at: v ? new Date(v).toISOString() : null })} />)}
            {cell(<ChipCell value={f.outcome} options={outcomeOptions(f.type)} placeholder="Set outcome" onChange={(v) => onField(f.id, { outcome: v })} />)}
            {cell(<ChipCell value={f.caller_name} options={staff.map(m => m.name)} placeholder="—" onChange={(v) => onField(f.id, { caller_name: v })} />)}
            {cell(<TextCell value={f.notes} placeholder="Add response…" onCommit={(v) => onField(f.id, { notes: v || null })} />)}
            {cell(<ChipCell value={f.status} options={['Scheduled', 'Completed', 'Missed', 'Rescheduled']} styleFor={(v) => statusStyle[v]} onChange={(v) => onField(f.id, { status: v })} />)}
          </div>
        ))}
        {/* Empty draft row */}
        <div className="grid items-stretch" style={{ gridTemplateColumns: FU_COLS, background: 'var(--color-surface-2)' }}>
          {cell(<ChipCell value={draft.type} options={types} placeholder="+ Type" styleFor={(v) => typeStyle[v]} onChange={(v) => touchDraft({ type: v })} />)}
          {cell(<TextCell value={draft.scheduled_at ? toLocalInput(draft.scheduled_at) : ''} type="datetime-local" onCommit={(v) => touchDraft({ scheduled_at: v ? new Date(v).toISOString() : null })} />)}
          {cell(<ChipCell value={draft.outcome} options={outcomeOptions(draft.type || 'Call')} placeholder="Set outcome" onChange={(v) => touchDraft({ outcome: v })} />)}
          {cell(<ChipCell value={draft.caller_name} options={staff.map(m => m.name)} placeholder="—" onChange={(v) => touchDraft({ caller_name: v })} />)}
          {cell(<TextCell value={draft.notes || ''} placeholder="Add response…" onCommit={(v) => touchDraft({ notes: v || null })} />)}
          {cell(<span className="px-2.5 py-2 text-[12px] self-center" style={{ color: 'var(--color-text-muted)' }}>new row</span>)}
        </div>
      </div>
    </div>
  )
