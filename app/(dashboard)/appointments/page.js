...
function CalendarView({
  ...
  useEffect(() => {
    const onMove = (e) => {
      if (hDrag.current && containerRef.current) {
        ...
        setCalWidth(
          Math.max(260, Math.min(e.clientX - rect.left, rect.width - 300)),
        );
      }
      if (vDrag.current.active) {
        const delta = e.clientY - vDrag.current.startY;
        setCalHeight(
          Math.max(380, Math.min(vDrag.current.startH + delta, 1000)),
        );
      }
    };
    const onUp = () => {
  ...
  const selectedAppts = dayAppts(selected).sort(
  ...
  return (
    <Card className="overflow-hidden">
      <div ref={containerRef} className="flex" style={{ height: calHeight }}>
          ...
          <div className="grid grid-cols-7 border-b border-(--color-border) shrink-0">
            {["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"].map((lbl, i) => (
              <div
                key={lbl}
                className={clsx(
                  "py-2 text-center text-[10px] font-700 uppercase tracking-wider",
          ...
          >
            {days.map((day, i) => {
              ...
              return (
                ...
                >
                  <span
                    className="text-[11px] font-600 leading-none self-end pr-0.5"
                    style={{
                      color: isSelected
                        ? "white"
                        : isCurrentDay
                  ...
                  {appts.length > 0 && (
                    <div className="flex flex-col gap-0.5 w-full">
                      <span
                        className="text-[9px] font-800 px-1.5 py-0.5 rounded self-start leading-none"
                        style={{
                          background: isSelected
        ...
        <div
          className="relative shrink-0 w-px group cursor-col-resize"
        ...
        >
          <div className="absolute inset-y-0 -left-2 -right-2 flex items-center justify-center z-10">
            <div
              className="flex flex-col items-center justify-center gap-0.5 w-3.5 h-8 rounded-full opacity-0 group-hover:opacity-100 transition-all shadow-sm"
              style={{ background: "var(--color-brand)" }}
            >
              {[0, 1, 2].map((i) => (
                <span
                  key={i}
                  className="w-0.5 h-0.5 rounded-full bg-white block"
                />
              ))}
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full">
          <div
            className="px-5 py-3.5 border-b border-(--color-border) shrink-0"
      ...
      <div
        className="relative h-1.75 border-t border-(--color-border) cursor-row-resize group transition-colors hover:bg-(--color-brand-50)"
        onMouseDown={(e) => {
          e.preventDefault();
          vDrag.current = {
            active: true,
...
export default function AppointmentsPage() {
  ...
  const [view, setView] = useState(() =>
    getPref("pref_appointments_view", "list"),
  );
  ...
  const filtered = appointments.filter((a) => {
    ...
    if (tab === "upcoming") {
      if (!(isFuture(d) || isToday(d)) || a.status === "cancelled")
        return false;
    }
    if (tab === "history") {
  ...
  const upcomingCount = appointments.filter(
    (a) => isFuture(new Date(a.scheduled_at)) && a.status !== "cancelled",
  ).length;
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p
            ...
            style={{ color: "var(--color-text-muted)" }}
          >
            {todayCount} today · {upcomingCount} upcoming · {completedCount}{" "}
            completed
          </p>
        </div>
        <div className="flex items-center gap-2">
          <div
            className="flex items-center gap-0.5 p-1 rounded-lg border border-(--color-border)"
            style={{ background: "var(--color-surface-2)" }}
          >
            {[
      ...
      {loading ? (
        ...
        <>
            ...
            {TABS.map((t) => (
              ...
              >
                ...
                {t.id === "today" && todayCount > 0 && (
                  <span
                    className="ml-1.5 text-[9px] px-1.5 py-0.5 rounded-full font-700"
          ...
          <div className="flex flex-wrap gap-2 items-center">
            {/* Search */}
            <div
              className="flex items-center gap-2 flex-1 min-w-52 px-3 py-2 rounded-xl border border-(--color-border) transition-colors focus-within:border-(--color-brand)"
              style={{ background: "var(--color-surface)" }}
            >
              <Search
                size={14}
          ...
          {showFilters && (
            <div
              className="flex flex-wrap gap-3 p-4 rounded-xl border border-(--color-border)"
              style={{ background: "var(--color-surface)" }}
            >
              {/* Doctor */}
              {doctors.length > 0 && (
