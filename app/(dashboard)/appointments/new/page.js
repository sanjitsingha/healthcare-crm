...
function TogglePill({ value, onChange, options }) {
  return (
    <div
      className="inline-flex rounded-xl border border-(--color-border) overflow-hidden"
...
export default function NewAppointmentPage() {
  ...
  useEffect(() => {
    if (!form.doctor_id) {
      setBilling((b) => ({ ...b, consultation_fee: "" }));
      return;
    }
    const doc = doctors.find((d) => (d.id || d.name) === form.doctor_id);
    if (doc?.fee != null)
      setBilling((b) => ({ ...b, consultation_fee: String(doc.fee) }));
  ...
  const cFee =
    billing.consultation_fee !== "" ? Number(billing.consultation_fee) : null;
  const rFee =
    newMode && billing.registration_fee !== ""
      ? Number(billing.registration_fee)
      : null;
  ...
  const dueLater =
    (!cPaidNow && cFee ? cFee : 0) + (!rPaidNow && rFee ? rFee : 0);
  ...
  const handleSubmit = async (e) => {
    ...
    if (newMode) {
      if (!newPat.first_name.trim()) {
        alert("First name is required.");
        return;
      }
      if (!newPat.phone.trim()) {
    ...
    try {
      ...
      if (newMode) {
        const created = await createPatient({
          ...
          custom_data: newPat.blood_group
            ? { blood_group: newPat.blood_group }
            : {},
      ...
      const appt = await createAppointment({
        ...
        consultation_fee_status:
          cFee != null ? (cPaidNow ? "paid" : "due") : null,
        registration_fee: rFee,
        registration_fee_status:
          rFee != null ? (rPaidNow ? "paid" : "due") : null,
      ...
      toast({
        ...
        message: appt?.appointment_code
          ? `Appointment ${appt.appointment_code} for ${name} has been booked.`
          ...
          {newMode && (
            <div className="grid grid-cols-4 gap-4">
              <Input
                ...
                onChange={(e) =>
                  setNewPat((p) => ({ ...p, first_name: e.target.value }))
                }
              />
              <Input
                label="Last Name"
                ...
                onChange={(e) =>
                  setNewPat((p) => ({ ...p, last_name: e.target.value }))
                }
              />
              <Input
                label="Phone *"
                ...
                value={newPat.phone}
                onChange={(e) =>
                  setNewPat((p) => ({ ...p, phone: e.target.value }))
                }
              />
              ...
              {moreOpen && (
                <>
                  <Input
                    ...
                    value={newPat.email}
                    onChange={(e) =>
                      setNewPat((p) => ({ ...p, email: e.target.value }))
                    }
                  />
                  <Input
                    label="Date of Birth"
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      setNewPat((p) => ({ ...p, blood_group: e.target.value }))
                    }
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewPat((p) => ({ ...p, address: e.target.value }))
                    }
                  />
                </>
              )}
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-wrap items-start gap-6">
              <div className="w-44 shrink-0">
                <label
                  className="block text-xs font-500 mb-1.5"
                  style={{ color: "var(--color-text-secondary)" }}
                >
                  Consultation Fee
                </label>
            ...
            {(billing.consult_when === "now" ||
              (newMode && billing.reg_when === "now")) && (
              <div
            ...
            {newMode && (
              <div className="pt-5 border-t border-(--color-border) flex flex-wrap items-start gap-6">
                <div className="w-44 shrink-0">
                  <label
                    className="block text-xs font-500 mb-1.5"
                    style={{ color: "var(--color-text-secondary)" }}
            ...
            {hasFees && (
              ...
              >
                ...
                <div className="px-4 py-3 space-y-2">
                  ...
                  {dueLater > 0 && (
                    <div className="flex items-center justify-between pt-2 mt-2 border-t border-(--color-border)">
                      <span
