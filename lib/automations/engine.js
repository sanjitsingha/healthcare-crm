...
async function executeAction(ctx, act) {
  ...
  try {
    switch (act.type) {
      ...
      case 'send_whatsapp': {
        if (!act.wa_template) { result.detail = 'no template selected'; return result }
        const phone = entity.whatsapp_phone || entity.phone
        if (!phone) { result.detail = 'no phone on record'; return result }
        // Fetch org WhatsApp config server-side (credentials never in rule payload).
        const { data: orgRow } = await supabase.from('organizations').select('settings').eq('id', orgId).single()
        const wa = orgRow?.settings?.whatsapp
        if (!wa?.enabled || !wa?.provider) { result.detail = 'WhatsApp not configured or disabled'; return result }
        // Resolve each positional param from the var_map saved on the action.
        const params = (act.wa_var_map || []).map(v => {
          if (!v || v.source === 'static') return String(v?.value ?? '')
          return String(getFieldValue(entity, v.field) ?? '')
        })
        const res = await sendWhatsApp({
          provider: wa.provider, config: wa,
          to: phone,
          templateName: act.wa_template,
          params,
          languageCode: act.wa_language || 'en_US',
        })
        if (!res.ok) throw new Error(res.error || `provider HTTP ${res.status}`)
        result.status = 'success'; result.detail = act.wa_template; return result
      }
      default:
