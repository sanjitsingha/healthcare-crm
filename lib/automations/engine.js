...
// ════════════════════════════════════════════════════════════════
// NOTE: WhatsApp sending is imported lazily inside runMessagingAutomations so
// the adapter never ships to the client (engine.js is also imported client-side
// via rulesEngine for condition evaluation).
...
// Resolve the WhatsApp recipient for an entity (best effort).
async function resolveRecipient(supabase, entity, links, toField) {
  if (toField && entity?.[toField]) return entity[toField]
  const direct = entity?.custom_data?.whatsapp_phone || entity?.whatsapp_phone || entity?.phone
  if (direct) return direct
  // appointment / consultation / task → look up the linked patient or lead phone
  const lookup = async (table, idVal) => {
    if (!idVal) return null
    const { data } = await supabase.from(table).select('phone, custom_data').eq('id', idVal).maybeSingle()
    return data?.custom_data?.whatsapp_phone || data?.phone || null
  }
  return (await lookup('patients', links?.patientId)) || (await lookup('leads', links?.leadId)) || null
}

// WhatsApp Automations (managed on the /automation page, table automation_rules).
// Evaluated for every event the engine runs; sends a WhatsApp template when the
// trigger + conditions match. Returns report rows for automation_runs history.
async function runMessagingAutomations({ supabase, org, event, entity, links, prev }) {
  const wa = org?.settings?.whatsapp
  const orgId = org?.id || entity?.organization_id
  if (!orgId || !wa?.provider || wa.enabled === false) return []

  let autos = []
  try {
    const { data } = await supabase.from('automation_rules')
      .select('id, name, conditions, actions, is_active, trigger_event')
      .eq('organization_id', orgId).eq('is_active', true).eq('trigger_event', event)
    autos = data || []
  } catch { return [] }

  if (!autos.length) return []
  // Lazy import keeps the WhatsApp adapter out of the client bundle.
  const { sendWhatsApp } = await import('@/lib/messaging/whatsapp')

  const out = []
  for (const a of autos) {
    const cond = a.conditions || {}
    const matched = evaluateConditions(entity, { conditions: cond.items || [], condition_match: cond.match || 'all', condition_groups: [] }, prev)
    if (!matched) continue
    const msg = (a.actions || [])[0]
    if (!msg || msg.type !== 'send_whatsapp') continue

    const actionResult = { type: 'send_whatsapp', status: 'skipped' }
    try {
      const to = await resolveRecipient(supabase, entity, links, msg.to_field)
      if (!to) { actionResult.detail = 'no recipient phone'; }
      else {
        const params = (msg.params || []).map(p => p?.source === 'static' ? (p.value ?? '') : (getFieldValue(entity, p?.value) ?? ''))
        const res = await sendWhatsApp({ provider: wa.provider, config: wa, to, templateName: msg.template, params, languageCode: msg.language })
        actionResult.status = res.ok ? 'success' : 'failed'
        actionResult.detail = res.ok ? `sent to ${to}` : null
        actionResult.error = res.ok ? null : (typeof res.error === 'string' ? res.error : JSON.stringify(res.response || res.error || 'send failed'))
      }
    } catch (err) { actionResult.status = 'failed'; actionResult.error = err.message }

    out.push({
      rule_id: a.id, rule_name: a.name || 'WhatsApp automation',
      target: 'whatsapp', event, actions: [actionResult], status: actionResult.status,
...
export async function runAutomations({ supabase, org, target, event, entity, prev, dryRun = false }) {
  ...
  // WhatsApp Automations (separate from Workflow Rules) — skip on dry runs.
  if (!dryRun) {
    const msgRows = await runMessagingAutomations({ supabase, org, event, entity, links, prev })
    report.push(...msgRows)
  }
