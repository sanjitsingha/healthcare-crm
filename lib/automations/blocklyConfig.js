...
export const ACTION_DEFS = {
  ...
  create_task:  { label: 'create task',         fields: [{ name: 'title', kind: 'text', ph: 'title' }, { name: 'priority', kind: 'priority' }, { name: 'dueMode', kind: 'duemode' }, { name: 'dueDays', kind: 'number', ph: 'days' }, { name: 'dueField', kind: 'datefield' }] },
...
const DUE_MODE_OPTS = [['in days from now', 'in'], ['days before', 'before'], ['days after', 'after']]
// Date fields per target, used for "N days before/after <date>" task scheduling.
const DATE_FIELDS = {
  lead: ['date_of_birth'],
  patient: ['date_of_birth'],
  appointment: ['scheduled_at'],
  consultation: ['consulted_at', 'follow_up_date'],
  task: ['due_date'],
}
...
function actionOptionsFor(kind) {
  switch (kind) {
    ...
    case 'duemode':  return DUE_MODE_OPTS
    case 'datefield':return asOpts(DATE_FIELDS[CTX.target] || [])
    default:         return [['—', '']]
  }
}
...
export function defineRuleBlocks() {
  ...
  Blockly.Blocks['condition_compare'] = {
    init() {
      this._val = ''            // single source of truth for the value
      this._pendingRebuild = false
      this.appendDummyInput('ROW')
        .appendField(new Blockly.FieldDropdown(conditionFieldOptions, this.onFieldChange_.bind(this)), 'FIELD')
        .appendField(new Blockly.FieldDropdown(RULE_OPS.map((o) => [o.label, o.value]), this.onOpChange_.bind(this)), 'OP')
        // VALUE starts as text so serialization.load can always apply the saved
        // value; rebuildValue_ then upgrades it to a dropdown when appropriate.
        .appendField(new Blockly.FieldTextInput('', this.onValueChange_.bind(this)), 'VALUE')
    ...
    // Changing the field changes what the value means → clear it.
    onFieldChange_(v) { this._val = ''; this.scheduleRebuild_(); return v },
    onOpChange_(v) { this.scheduleRebuild_(); return v },
    onValueChange_(v) { this._val = v ?? ''; return v },
    // Coalesce rapid FIELD+OP changes (e.g. during load) into one rebuild that
    // runs after the field values have committed.
    scheduleRebuild_() {
      if (this._pendingRebuild) return
      this._pendingRebuild = true
      setTimeout(() => { this._pendingRebuild = false; this.rebuildValue_() }, 0)
    },
    rerender_() { (this.queueRender ? this.queueRender() : this.render && this.render()) },
    ...
    rebuildValue_() {
      ...
      // Capture whatever is currently typed/selected before removing the field.
      if (this.getField('VALUE')) {
        this._val = this.getFieldValue('VALUE') ?? this._val
        try { row.removeField('VALUE') } catch {}
      }
      // Empty / not-empty checks take no value.
      if (!OP_NEEDS_VALUE[op]) { this.rerender_(); return }

      const opts = FREEFORM_OPS.has(op) ? null : fieldOptions(CTX.target, field, { stages: CTX.stages })
      if (opts && opts.length) {
        const pairs = [['Select…', ''], ...opts.map((o) => [String(o), String(o)])]
        const dd = new Blockly.FieldDropdown(pairs, this.onValueChange_.bind(this))
        row.appendField(dd, 'VALUE')
        // Restore only if the saved value is a valid option (avoids Blockly's
        // "unknown dropdown value" warning); otherwise fall back to the default.
        if (pairs.some((p) => p[1] === this._val)) {
          try { this.setFieldValue(this._val, 'VALUE') } catch {}
        } else {
          this._val = this.getFieldValue('VALUE') || ''
        }
      } else {
        // Free-form ops (>, <, between, contains, …) and non-option fields use
        // a text input. (between takes "min,max"; engine coerces numbers.)
        const txt = new Blockly.FieldTextInput(this._val || '', this.onValueChange_.bind(this))
        row.appendField(txt, 'VALUE')
      }
      this.rerender_()
    },
  }
  ...
  // If / Else control flow — branches the action list based on conditions.
  Blockly.Blocks['action_if'] = {
    init() {
      this.appendDummyInput()
        .appendField('IF · match')
        .appendField(new Blockly.FieldDropdown(MATCH_OPTS), 'MATCH')
      this.appendStatementInput('COND').setCheck('Condition').appendField('when')
      this.appendStatementInput('THEN').setCheck('Action').appendField('then')
      this.appendStatementInput('ELSE').setCheck('Action').appendField('else')
      this.setPreviousStatement(true, 'Action')
      this.setNextStatement(true, 'Action')
      this.setColour('#db2777')
      this.setTooltip('If the conditions match, run the THEN actions; otherwise run the ELSE actions.')
    },
  }
}
