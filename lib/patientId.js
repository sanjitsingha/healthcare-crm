import { format as fmtDate } from 'date-fns'

// Date token → date-fns pattern
export const DATE_FORMATS = {
  YYYYMMDD:  { label: 'YYYYMMDD', pattern: 'yyyyMMdd' },
  YYMMDD:    { label: 'YYMMDD',   pattern: 'yyMMdd' },
  YYYYMM:    { label: 'YYYYMM',   pattern: 'yyyyMM' },
  YYYY:      { label: 'YYYY',     pattern: 'yyyy' },
  YY:        { label: 'YY',       pattern: 'yy' },
  DDMMYY:    { label: 'DDMMYY',   pattern: 'ddMMyy' },
}

export const SEPARATORS = [
  { value: '-', label: 'Dash ( - )' },
  { value: '/', label: 'Slash ( / )' },
  { value: '.', label: 'Dot ( . )' },
  { value: '_', label: 'Underscore ( _ )' },
  { value: ' ', label: 'Space' },
]

export const DEFAULT_ID_FORMAT = {
  enabled: false,
  next_seq: 1,
  tokens: [
    { id: 't1', type: 'text', value: 'PT' },
    { id: 't2', type: 'separator', value: '-' },
    { id: 't3', type: 'date', value: 'YYYYMMDD' },
    { id: 't4', type: 'separator', value: '-' },
    { id: 't5', type: 'seq', value: 4 },
  ],
}

// Default auto-ID format for pharmacy inventory items → MED-0001
export const DEFAULT_PHARMACY_ID_FORMAT = {
  enabled: true,
  next_seq: 1,
  tokens: [
    { id: 'p1', type: 'text', value: 'MED' },
    { id: 'p2', type: 'separator', value: '-' },
    { id: 'p3', type: 'seq', value: 4 },
  ],
}

// Default auto-ID format for pharmacy sale invoices → INV-0001
export const DEFAULT_PHARMACY_INVOICE_FORMAT = {
  enabled: true,
  next_seq: 1,
  tokens: [
    { id: 'i1', type: 'text', value: 'INV' },
    { id: 'i2', type: 'separator', value: '-' },
    { id: 'i3', type: 'seq', value: 4 },
  ],
}

// Convert legacy (prefix/date_format/separator/seq_digits) config to tokens
export function toTokens(cfg = {}) {
  if (Array.isArray(cfg.tokens) && cfg.tokens.length) return cfg.tokens
  const sep = cfg.separator && cfg.separator !== 'none' ? cfg.separator : '-'
  const tokens = []
  const uid = () => Math.random().toString(36).slice(2)
  if (cfg.prefix) tokens.push({ id: uid(), type: 'text', value: cfg.prefix })
  if (cfg.date_format && cfg.date_format !== 'none' && DATE_FORMATS[cfg.date_format]) {
    if (tokens.length) tokens.push({ id: uid(), type: 'separator', value: sep })
    tokens.push({ id: uid(), type: 'date', value: cfg.date_format })
  }
  if (tokens.length) tokens.push({ id: uid(), type: 'separator', value: sep })
  tokens.push({ id: uid(), type: 'seq', value: cfg.seq_digits || 4 })
  return tokens
}

// Derive a short code from a doctor object.
//   format='initials' → first letter of each name word (e.g. "Dr. Anita Sharma" → "AS")
//   format='name'     → last word uppercased, max 6 chars  (e.g. "Dr. Anita Sharma" → "SHARMA")
export function getDoctorCode(doctor, format = 'initials') {
  const raw = (doctor?.name || doctor?.full_name || '').replace(/^Dr\.?\s*/i, '').trim()
  if (!raw) return ''
  if (format === 'name') {
    const parts = raw.split(/\s+/).filter(Boolean)
    return (parts[parts.length - 1] || parts[0] || '').toUpperCase().slice(0, 6)
  }
  // initials (default)
  return raw.split(/\s+/).filter(Boolean).map(w => w[0].toUpperCase()).join('')
}

// Build a code from the token list + sequence number.
// Pass { doctorCode } when the format includes a 'doctor' token.
export function buildPatientCode(cfg = {}, seq = 1, date = new Date(), { doctorCode = '' } = {}) {
  const tokens = toTokens(cfg)
  return tokens.map(tok => {
    if (tok.type === 'text')      return tok.value || ''
    if (tok.type === 'separator') return tok.value || ''
    if (tok.type === 'date')      { const p = DATE_FORMATS[tok.value]?.pattern; return p ? fmtDate(date, p) : '' }
    if (tok.type === 'seq')       return String(seq).padStart(Number(tok.value) || 4, '0')
    if (tok.type === 'doctor')    return doctorCode || '{DR}'
    return ''
  }).join('')
}
