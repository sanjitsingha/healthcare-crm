...
export async function generateOrgApiKey(orgId, currentSettings) {
  const bytes = new Uint8Array(16)
  crypto.getRandomValues(bytes)
  const key = 'hcrm_' + Array.from(bytes, b => b.toString(16).padStart(2, '0')).join('')
  await updateOrganization(orgId, { settings: { ...(currentSettings || {}), public_api_key: key } })
  return key
}
