...
import {
  ...
  getDoctorCode,
  toTokens,
  DEFAULT_PHARMACY_ID_FORMAT,
  DEFAULT_PHARMACY_INVOICE_FORMAT,
} from "../patientId";
...
const logDelete = (entityType, id, error) =>
  logAudit({
    ...
    entityId: id,
    description: error
      ? `${entityType} deletion failed: ${error.message}`
...
export async function getOrganization(id) {
  const { data, error } = await supabase
    .from("organizations")
    .select("*, contacts!organization_id(*), leads(*)")
    .eq("id", id)
    .single();
  if (error) throw error;
  return data;
...
export async function getPatient(id) {
  const { data, error } = await supabase
    .from("patients")
    .select(
      "*, leads(*), appointments(*), invoices(*), tags:patient_tags(tags(*))",
    )
    .eq("id", id)
    .single();
  if (error) throw error;
    ...
    try {
      ...
      if (cfg?.enabled) {
        ...
        if (code)
          nextSettings = {
            ...orgRow.settings,
            patient_id_format: { ...cfg, next_seq: seq + 1 },
          };
      }
    } catch {
      /* ignore — fall back to no code */
    }
  }
...
export async function deletePatient(id) {
  ...
  await logAudit({
    ...
    entityId: id,
    description: error
      ? `Patient deletion failed: ${error.message}`
...
export async function getLeads({
  ...
  let q = supabase
    .from("leads")
    .select(
      "*, contacts(id, first_name, last_name, email, phone), patients(id, first_name, last_name, phone, email, gender, address, patient_code), tags:lead_tags(tags(*))",
    )
    .eq("organization_id", orgId)
    .order("created_at", { ascending: false });
  const { data, error } = await q;
...
export async function getLead(id) {
  const { data, error } = await supabase
    .from("leads")
    .select(
      "*, contacts(id, first_name, last_name, email, phone), patients(id, first_name, last_name, phone, email, gender, date_of_birth, address), tags:lead_tags(tags(*))",
    )
    .eq("id", id)
    .single();
  if (error) throw error;
...
export async function getPersonTimeline({
  ...
  const allActs = lists
    .flat()
    .filter(
      (a) => !(patientId && /converted to patient/i.test(a.content || "")),
    );
  ...
  if (patientId) {
    const alreadyLogged = allActs.some(
      (a) =>
        a.entity_id === patientId && /patient created/i.test(a.content || ""),
    );
    if (!alreadyLogged) {
...
export async function createAppointment(appointment) {
  ...
  if (appointment.organization_id) {
    try {
      const { data: orgRow } = await supabase
        ...
        .single();
      const cfg = orgRow?.settings?.appointment_id_format;
      if (cfg?.enabled) {
        const tokens = toTokens(cfg);
        const docTok = tokens.find((t) => t.type === "doctor");
        const doctorId = appointment.doctor_id || null;
        let seq,
          doctorCode = "";

        if (docTok && doctorId) {
          // Per-doctor sequence: each doctor owns their own counter.
          const seqMap = cfg.sequences || {};
          seq = Number(seqMap[doctorId] ?? cfg.next_seq ?? 1);
          const doctor = (orgRow.settings.doctors || []).find(
            (d) => (d.id || d.name) === doctorId,
          );
          doctorCode = getDoctorCode(doctor, docTok.value || "initials");
          nextSettings = {
            ...orgRow.settings,
            appointment_id_format: {
              ...cfg,
              sequences: { ...seqMap, [doctorId]: seq + 1 },
            },
          };
        } else {
          // Global sequence (no doctor token, or appointment has no doctor).
          seq = Number(cfg.next_seq ?? 1);
          nextSettings = {
            ...orgRow.settings,
            appointment_id_format: { ...cfg, next_seq: seq + 1 },
          };
        }
        ...
        code = buildPatientCode(
          cfg,
          seq,
          new Date(appointment.scheduled_at || Date.now()),
          { doctorCode },
        );
      }
    } catch {
      /* non-fatal — proceed without code */
    }
  }

  const payload = { ...appointment, appointment_code: code };
  let { data, error } = await supabase
    .from("appointments")
    .insert(payload)
    .select()
  ...
  // Graceful fallback if appointment_code column doesn't exist yet.
  if (error && /appointment_code/i.test(error.message || "")) {
    const { appointment_code, ...rest } = payload;
    ({ data, error } = await supabase
      .from("appointments")
      .insert(rest)
      .select()
  ...
  // Persist the incremented counter(s) back to org settings.
  if (nextSettings && appointment.organization_id) {
    try {
      await supabase
        .from("organizations")
        .update({ settings: nextSettings })
...
export async function getConsultations({
  ...
  let q = supabase
    .from("consultations")
    .select(
      "*, patients(id, first_name, last_name), leads(id, title, first_name, last_name)",
    )
    .eq("organization_id", orgId)
...
export async function getInvoices({ orgId = getOrgId() } = {}) {
  ...
  return (invRes.data || []).map((inv) => {
    ...
    if (inv.status !== "Cancelled") {
      if (paid_amount >= (inv.total || 0) && (inv.total || 0) > 0)
...
export async function getTransactions({ orgId = getOrgId() } = {}) {
  if (!orgId) return [];
  const [invoices, sales, consultations, appointments, patientList] =
    await Promise.all([
  ...
  for (const i of invoices) {
    ...
    const extra =
      (i.invoice_items?.length || 0) > 1
        ? ` +${i.invoice_items.length - 1}`
        : "";
    tx.push({
      id: `inv-${i.id}`,
      date: i.created_at,
  ...
  for (const a of appointments) {
    ...
    if (cf > 0)
      tx.push({
        ...
        amount: cf,
        paid: null,
        status: a.consultation_fee_status
          ? cap(a.consultation_fee_status)
          : "Due",
      });
    if (rf > 0)
      tx.push({
        ...
        amount: rf,
        paid: null,
        status: a.registration_fee_status
          ? cap(a.registration_fee_status)
          : "Due",
      });
  }
...
export async function getDashboardStats(
  orgId = getOrgId(),
  { from = null, to = null, assignedTo = null } = {},
) {
  if (!orgId)
    return {
      totalLeads: 0,
  ...
  const inRange = (q) =>
    hasRange ? q.gte("created_at", fromIso).lt("created_at", toIso) : q;
  ...
  const { data: valueData } = await byUser(
    inRange(
      supabase
        .from("leads")
        .select("value")
  ...
  const totalValue =
    valueData?.reduce((sum, l) => sum + (l.value || 0), 0) || 0;

  const { data: stageData } = await byUser(
    inRange(
  ...
  const { data: sourceData } = await byUser(
    inRange(
  ...
  const startTodayIso = iso(startToday),
    endTodayIso = iso(endToday);
  ...
  const [
    ...
    byUser(
      supabase
        ...
        .eq("organization_id", orgId)
        .eq("status", "Pending")
        .gte("due_date", startTodayIso)
        .lt("due_date", endTodayIso),
    ),
    supabase
      .from("followups")
      .select("*", { count: "exact", head: true })
...
export async function peekPharmacyItemCode(orgId) {
  ...
  const { data: orgRow } = await supabase
    ...
    .single();
  const cfg =
    orgRow?.settings?.pharmacy_item_id_format || DEFAULT_PHARMACY_ID_FORMAT;
    ...
    try {
      ...
      if (cfg?.enabled) {
        ...
        if (code)
          nextSettings = {
            ...(orgRow?.settings || {}),
            pharmacy_item_id_format: { ...cfg, next_seq: seq + 1 },
          };
      }
    } catch {
      /* ignore — fall back to no code */
...
export async function getPharmacySale(id) {
  const { data, error } = await supabase
    .from("pharmacy_sales")
    .select(
      "*, patients(id, first_name, last_name, patient_code, gender, date_of_birth, address, phone), pharmacy_sale_items(*)",
    )
    .eq("id", id)
    .single();
  if (error) throw error;
...
async function decrementPharmacyStock(items = []) {
  ...
  try {
    ...
    const qtyMap = Object.fromEntries(
      (current || []).map((r) => [r.id, Number(r.quantity) || 0]),
    );
    await Promise.all(
      withIds.map((it) => {
        const next = Math.max(
          0,
          (qtyMap[it.item_id] ?? 0) - (Number(it.quantity) || 0),
        );
        return supabase
          .from("pharmacy_items")
    ...
    /* best-effort; sale already recorded */
  }
}
...
export async function savePharmacySale({
  id = null,
  sale,
  items = [],
  complete = false,
  ...
  if (complete && !code) {
    try {
      const { data: orgRow } = await supabase
        ...
        .single();
      const cfg =
        orgRow?.settings?.pharmacy_invoice_id_format ||
        DEFAULT_PHARMACY_INVOICE_FORMAT;
      if (cfg?.enabled) {
        const seq = Number(cfg.next_seq) || 1;
        code = buildPatientCode(cfg, seq);
        if (code)
          nextSettings = {
            ...(orgRow?.settings || {}),
            pharmacy_invoice_id_format: { ...cfg, next_seq: seq + 1 },
          };
      }
    } catch {
      /* ignore — fall back to no code */
