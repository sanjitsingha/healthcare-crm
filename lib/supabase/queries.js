...
import { triggerAutomation } from '../automations/trigger'
...
export async function createAppointment(appointment) {
  ...
  if (error) throw error
  // Fire appointment-target automation rules (e.g. reminder tasks). The
  // appointment is the entity, so rules can schedule relative to scheduled_at.
  if (data?.organization_id) {
    triggerAutomation({ orgId: data.organization_id, target: 'appointment', event: 'appointment_booked', entityId: data.id })
  }
  return data
}
...
export async function getInvoices({ orgId = getOrgId() } = {}) {
  ...
  const [invRes, pmtRes] = await Promise.all([
    supabase.from('invoices')
      .select('*, patients(id, first_name, last_name), invoice_items(*)')
      .eq('organization_id', orgId)
      .order('created_at', { ascending: false }),
  ...
  for (const p of (pmtRes.data || [])) {
    pmtMap[p.invoice_id] = (pmtMap[p.invoice_id] || 0) + (p.amount || 0)
  }
  const today = new Date(); today.setHours(0, 0, 0, 0)
  return (invRes.data || []).map(inv => {
    const paid_amount = pmtMap[inv.id] || 0
    const balance = (inv.total || 0) - paid_amount
    let effective_status = inv.status || 'Draft'
    if (inv.status !== 'Cancelled') {
      if (paid_amount >= (inv.total || 0) && (inv.total || 0) > 0) effective_status = 'Paid'
      else if (paid_amount > 0) effective_status = 'Partially Paid'
      else if (inv.due_date && new Date(inv.due_date) < today) effective_status = 'Overdue'
    }
    return { ...inv, paid_amount, balance, effective_status }
  })
}
...
export async function createInvoice(invoice) {
  ...
  if (items?.length) {
    ...
    if (ie) console.error('invoice_items error:', ie)
  }
  return data
}
...
export async function recordPayment(payment) {
  ...
  if (error) throw error
  const [{ data: pmts }, { data: inv }] = await Promise.all([
    supabase.from('payments').select('amount').eq('invoice_id', payment.invoice_id),
    supabase.from('invoices').select('total, status').eq('id', payment.invoice_id).single(),
  ])
  if (inv && inv.status !== 'Cancelled' && pmts) {
    const totalPaid = pmts.reduce((s, p) => s + (p.amount || 0), 0)
    const newStatus = totalPaid >= (inv.total || 0) ? 'Paid' : 'Partially Paid'
...
// Aggregates every billable charge across sources (invoices/billing,
// pharmacy sales, doctor consultations, appointment fees) into a single
// normalized list with a `source` so the billing → Transactions view can
// show "where each transaction came from".
export async function getTransactions({ orgId = getOrgId() } = {}) {
  if (!orgId) return []
  const [invoices, sales, consultations, appointments, patientList] = await Promise.all([
    getInvoices({ orgId }).catch(() => []),
    getPharmacySales({ orgId, limit: 1000 }).catch(() => []),
    getConsultations({ orgId }).catch(() => []),
    getAppointments({ orgId }).catch(() => []),
    getPatients({ orgId }).catch(() => []),
  ])

  const phoneMap = {}
  for (const p of patientList) phoneMap[p.id] = p.phone || ''
  const phoneOf = (pt) => (pt && pt.id ? (phoneMap[pt.id] || '') : '')
  const name = (p) => p ? `${p.first_name || ''} ${p.last_name || ''}`.trim() : ''
  const cap = (s) => s ? String(s).charAt(0).toUpperCase() + String(s).slice(1) : ''
  const tx = []

  // Billing / invoices — one row per invoice (amount = total, paid from payments)
  for (const i of invoices) {
    const first = i.invoice_items?.[0]?.description
    const extra = (i.invoice_items?.length || 0) > 1 ? ` +${i.invoice_items.length - 1}` : ''
    tx.push({
      id: `inv-${i.id}`,
      date: i.created_at,
      source: 'Billing',
      reference: i.invoice_number || '—',
      patient: name(i.patients) || '—',
      phone: phoneOf(i.patients),
      description: first ? `${first}${extra}` : 'Invoice',
      amount: Number(i.total) || 0,
      paid: Number(i.paid_amount) || 0,
      status: i.effective_status || i.status || 'Draft',
    })
  }
  ...
  for (const s of sales) {
    ...
    let status
    if (s.status === 'draft') status = 'Draft'
    else if (total > 0 && paid >= total) status = 'Paid'
    else if (paid > 0) status = 'Partially Paid'
    else status = s.payment_status ? cap(s.payment_status) : 'Unpaid'
    const count = s.pharmacy_sale_items?.length || 0
    tx.push({
      id: `pho-${s.id}`,
      date: s.created_at,
      source: 'Pharmacy',
      reference: s.sale_code || (s.status === 'draft' ? 'Draft' : '—'),
      patient: name(s.patients) || 'Walk-in',
      phone: phoneOf(s.patients),
      description: count ? `${count} item${count > 1 ? 's' : ''}` : 'Pharmacy sale',
      amount: total,
      paid,
      status,
    })
  ...
  // Doctor consultations (skip zero-fee records)
  for (const c of consultations) {
    const amount = Number(c.amount) || 0
    if (amount <= 0) continue
    tx.push({
      id: `con-${c.id}`,
      date: c.consulted_at || c.created_at,
      source: 'Consultation',
      reference: '—',
      patient: name(c.patients) || name(c.leads) || '—',
      phone: phoneOf(c.patients),
      description: 'Doctor consultation fee',
      amount,
      paid: null,
      status: c.status ? cap(c.status) : 'Pending',
    })
  }

  // Appointment fees — separate row per non-zero fee, each with its own status
  for (const a of appointments) {
    const pname = name(a.patients) || name(a.leads) || '—'
    const cf = Number(a.consultation_fee) || 0
    const rf = Number(a.registration_fee) || 0
    const pphone = phoneOf(a.patients)
    if (cf > 0) tx.push({
      id: `apt-c-${a.id}`, date: a.created_at, source: 'Appointment', reference: '—',
      patient: pname, phone: pphone, description: 'Consultation fee', amount: cf,
      paid: null, status: a.consultation_fee_status ? cap(a.consultation_fee_status) : 'Due',
    })
    if (rf > 0) tx.push({
      id: `apt-r-${a.id}`, date: a.created_at, source: 'Appointment', reference: '—',
      patient: pname, phone: pphone, description: 'Registration fee', amount: rf,
      paid: null, status: a.registration_fee_status ? cap(a.registration_fee_status) : 'Due',
    })
  }

  tx.sort((x, y) => new Date(y.date) - new Date(x.date))
  return tx
}
...
// ── Automation run log ─────────────────────────────────────────
export async function getAutomationRuns({ orgId = getOrgId(), ruleId = '', limit = 50 } = {}) {
  if (!orgId) return []
  let q = supabase
  ...
  const { data, error } = await q
  if (error) {
    // Table may not be migrated yet — don't break the Rules page.
...
// ── Dashboard Stats ────────────────────────────────────────────
// Range-aware dashboard stats. Pass { from, to } (Date or ISO) to scope the
// acquisition metrics (leads, patients, revenue, conversion, funnel) to a window;
// omit for all-time. Pass `assignedTo` to scope by the owning user (leads,
// patients, pending tasks). Current-state metrics (upcoming/today appts) are
// always "as of now". Trends compare the window to the previous equal period.
export async function getDashboardStats(orgId = getOrgId(), { from = null, to = null, assignedTo = null } = {}) {
  ...
  const fromIso = hasRange ? new Date(from).toISOString() : null
  const toIso   = hasRange ? new Date(to).toISOString() : null
  const inRange = q => hasRange ? q.gte('created_at', fromIso).lt('created_at', toIso) : q
  const byUser  = q => assignedTo ? q.eq('assigned_to', assignedTo) : q
  ...
  const [
    ...
    byUser(inRange(supabase.from('leads').select('*', { count: 'exact', head: true }).eq('organization_id', orgId))),
    byUser(inRange(supabase.from('leads').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).eq('stage', 'Converted'))),
    supabase.from('contacts').select('*', { count: 'exact', head: true }).eq('organization_id', orgId),
    byUser(inRange(supabase.from('patients').select('*', { count: 'exact', head: true }).eq('organization_id', orgId))),
    byUser(supabase.from('tasks').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).eq('status', 'Pending')),
    supabase.from('followups').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).eq('status', 'Scheduled'),
  ])

  const { data: valueData } = await byUser(inRange(
    supabase.from('leads').select('value').eq('organization_id', orgId).eq('stage', 'Converted')
  ))
  const totalValue = valueData?.reduce((sum, l) => sum + (l.value || 0), 0) || 0

  const { data: stageData } = await byUser(inRange(
    supabase.from('leads').select('stage').eq('organization_id', orgId)
  ))
  const stageCounts = {}
  stageData?.forEach(l => { stageCounts[l.stage] = (stageCounts[l.stage] || 0) + 1 })

  // Lead sources (range + user aware) — ranked breakdown.
  const { data: sourceData } = await byUser(inRange(
    supabase.from('leads').select('source').eq('organization_id', orgId)
  ))
  const sourceCounts = {}
  sourceData?.forEach(l => { const s = l.source || 'Unknown'; sourceCounts[s] = (sourceCounts[s] || 0) + 1 })
  const sources = Object.entries(sourceCounts).map(([source, count]) => ({ source, count })).sort((a, b) => b.count - a.count)

  // Current-state counts (always "now", independent of the date range).
  const now = new Date()
  const iso = d => d.toISOString()
  const startToday = new Date(now); startToday.setHours(0, 0, 0, 0)
  const endToday = new Date(startToday); endToday.setDate(endToday.getDate() + 1)
  const startTodayIso = iso(startToday), endTodayIso = iso(endToday)

  const [
    { count: appointmentsUpcoming }, { count: appointmentsToday },
    { count: newLeadsToday }, { count: tasksDueToday }, { count: followupsDueToday },
    { count: overdueTasks }, { count: missedFollowups }, { count: unassignedLeads },
  ] = await Promise.all([
    supabase.from('appointments').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).in('status', ['booked', 'confirmed']).gte('scheduled_at', iso(now)),
    supabase.from('appointments').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).gte('scheduled_at', startTodayIso).lt('scheduled_at', endTodayIso),
    // Today's focus
    byUser(supabase.from('leads').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).gte('created_at', startTodayIso).lt('created_at', endTodayIso)),
    byUser(supabase.from('tasks').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).eq('status', 'Pending').gte('due_date', startTodayIso).lt('due_date', endTodayIso)),
    supabase.from('followups').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).eq('status', 'Scheduled').gte('scheduled_at', startTodayIso).lt('scheduled_at', endTodayIso),
    // Needs attention
    byUser(supabase.from('tasks').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).eq('status', 'Pending').lt('due_date', startTodayIso)),
    supabase.from('followups').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).eq('status', 'Scheduled').lt('scheduled_at', startTodayIso),
    supabase.from('leads').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).is('assigned_to', null).not('stage', 'in', '(Converted,Lost)'),
  ])

  // Trends vs the previous equal-length period (only for a bounded range).
  let leadsTrend, patientsTrend
  if (hasRange) {
    const dur = new Date(toIso).getTime() - new Date(fromIso).getTime()
    const prevFrom = new Date(new Date(fromIso).getTime() - dur).toISOString()
    const [{ count: leadsPrev }, { count: patientsPrev }] = await Promise.all([
      byUser(supabase.from('leads').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).gte('created_at', prevFrom).lt('created_at', fromIso)),
      byUser(supabase.from('patients').select('*', { count: 'exact', head: true }).eq('organization_id', orgId).gte('created_at', prevFrom).lt('created_at', fromIso)),
    ])
    const trend = (cur, prev) => prev ? Math.round(((cur - prev) / prev) * 100) : (cur ? 100 : 0)
    leadsTrend = trend(totalLeads || 0, leadsPrev || 0)
    patientsTrend = trend(totalPatients || 0, patientsPrev || 0)
  }
